    ...
    <div class="row gutters-10">
        <div class="col-md-3">
            <div class="bg-grad-3 text-white rounded-lg mb-4 overflow-hidden">
              <div class="px-3 pt-3">
                <div class="h3 fw-700">
                  {{ count(\App\Models\Song::where('artist_id', Auth::user()->id)->get()) }}
                </div>
        ...
        <div class="col-md-3">
            <div class="bg-grad-1 text-white rounded-lg mb-4 overflow-hidden">
                <div class="px-3 pt-3">
                    <div class="h3 fw-700">
                      {{ count(\App\Models\Album::where('artist_id', Auth::user()->id)->get()) }}
                    </div>
                    <div class="opacity-50">{{ translate('Album')}}</div>
                </div>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
                ...
                <table class="table aiz-table mb-0">
                  ...
                  <tbody>
                    @foreach (\App\Models\Category::all() as $key => $category)
                        @if(count($category->songs->where('artist_id', Auth::user()->id))>0)
                          <tr>
                              <td>{{ $category->getTranslation('name') }}</td>
                              <td>{{ count($category->songs->where('artist_id', Auth::user()->id)) }}</td>
                          </tr>
                      @endif
                  @endforeach
                </table>
