...
class PurchaseHistoryController extends Controller
{
    public function index($id, Request $request)
    {
        ...
        $hot_orders = DB::table('hot_orders')
            ->select("id", DB::RAW('"NULL" AS code'), "user_id", "delivery_status", DB::RAW('"NULL" AS payment_type'), "payment_status", "grand_total", DB::RAW("order_at AS date"), DB::RAW("'hot_order' AS type"))
            ->where("user_id", $id);
        ...
        foreach ($data_order->items() as &$data){
            ...
            if($data->type == 'hot_order'){
                ...
                if($hot_order){
                    ...
                    $seller = Seller::where('user_id', $hot_order->seller_id)->first();
                    // dd($hot_order->seller_id);
                }
                ...
                $data->type_details = [
                    ...
                    'shop_name' => $seller?$seller->user->shop->name:'',
