...
class HomeController extends Controller
{
    ...
    public function loginToken(Request $request){
        if ($request->isMethod('post')) {
            $data = $request->all();
            $infoUser = \App\Helpers\Hash::openssl_decrypt($data["token"]);
            $infoUser = json_decode($infoUser);
            $existingUser = \App\User::where('email', $infoUser->email)
                ->where('password', $infoUser->password)
                ->first();
            if($existingUser){
                // log them in
                auth()->login($existingUser, true);
                return redirect($data["src"]);
            }
        }
